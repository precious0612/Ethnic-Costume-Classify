...
class Picture:
    ...
    def get_one_html(self, url, pn):
        response = requests.get(url=url.format(self.name, self.name, pn, self.times),
                                headers=self.headers, proxies=proxies).content.decode('utf-8')
        return response

    # 请求单张图片内容  获取指定图片的UR
    def get_two_html(self, url):
        response = requests.get(url=url, headers=self.headers, proxies=proxies).content
        return response
